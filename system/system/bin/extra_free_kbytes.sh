...
# MIUI ADD: Performance_MemoryPolicyEnhance
...
then
    ...
    if [ $RamSizeGB -gt 6 ]; then
        max_watermark_scale=$((vendor_watermark_scale_factor + 7))
    fi
    if [ $RamSizeGB -gt 8 ]; then
        max_watermark_scale=$vendor_watermark_scale_factor
    fi
    if [ $RamSizeGB -gt 16 ]; then
        max_watermark_scale=$((2 * $max_watermark_scale))
    fi
fi
# END Performance_MemoryPolicyEnhance
